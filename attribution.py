...
Step 2 matches on ``lower(trim(...))``, not bare ``lower(...)``. The sheet is
hand-maintained, and a single trailing space on a Lead SE cell was enough to
drop the match — which does not surface as an error but as the deal quietly
reporting "Unassigned" and its ARR vanishing from that rep's total. Both sides
are trimmed, and ``db.py`` indexes ``lower(trim(name))`` to match, so the
tolerance costs nothing at query time. Anything beyond whitespace ("NicDaSilva",
"Da Silva, Nic") is deliberately NOT matched: guessing at a different spelling
risks attributing revenue to the wrong person. Those surface via
``lead_se_unmatched`` instead.
...
EFFECTIVE_SE_ID_SQL = """COALESCE(
    ...
    (SELECT r2.id FROM se_reps r2
        WHERE lower(trim(r2.name)) = lower(trim(tf.lead_se_name)) LIMIT 1),
...
LEAD_SE_ID_SQL = """(SELECT r.id FROM se_reps r
    WHERE lower(trim(r.name)) = lower(trim(tf.lead_se_name)) LIMIT 1)"""
